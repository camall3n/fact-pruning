...
CONFIG_NICKS.append(('basic', ["--translate-options", "--search-options", "--search", "astar(lmcut())"]))
for x in ["VR", "FR", "FCR", "FCMR"]:
    CONFIG_NICKS.append((x, ["--translate-options", '--keep-unimportant-variables', '--keep-unreachable-facts' ,"--scoping", x, "--search-options", "--search", "astar(lmcut())"]))
...
SUITE = common_setup.DEFAULT_2023_SUITE
...
def rename_algorithms(run):
    ...
    for a in ["VR", "FR", "FCR", "FCMR"]:
        paper_names[f"{REVISIONS[0]}-{a}"] = a
    run["algorithm"] = paper_names[name]
    return run

algos = ["No scoping", "FD+R", "VR", "FR", "FCR", "FDMR"]
